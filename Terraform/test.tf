...
resource "aws_instance" "airflow_server" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo sed -i 's|.*broker_url =.*|broker_url = amqp://airflow_user:airflow@${self.private_ip}:5672/|' /home/ubuntu/airflow/airflow.cfg",
...
resource "aws_instance" "airflow_worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo sed -i 's|.*broker_url =.*|broker_url = amqp://airflow_user:airflow@${aws_instance.airflow_server.private_ip}:5672/|' /home/ubuntu/airflow/airflow.cfg",
