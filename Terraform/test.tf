...
resource "aws_instance" "airflow_server" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo sed -i 's|.*broker_url =.*|broker_url = amqp://admin:admin@${self.private_ip}:5672/|' /home/ubuntu/airflow/airflow.cfg",
      "sudo sudo supervisorctl restart airflowserver",
...
resource "aws_instance" "airflow_worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo sed -i 's|.*broker_url =.*|broker_url = amqp://admin:admin@${aws_instance.airflow_server.private_ip}:5672/|' /home/ubuntu/airflow/airflow.cfg",
      "sudo sudo supervisorctl restart airflowworker"
    ]
  }
